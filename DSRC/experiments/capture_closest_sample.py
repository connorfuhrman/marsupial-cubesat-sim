...
class Experiment(Simulation):
    ...
    def _update_samples(self):
        if np.random.uniform() > 0.5 and \
           self.num_samples < self._max_num_samples and \
           self.simtime < 5*60:
    ...
    def _on_first_iter(self):
        if self.num_motherships > 1:
            self._logger.warn("There are %s motherships but this is really only set "
    ...
    def _do_cubesat_deploy(self, mship):
        ...
        to_deploy = CubeSat(deploy_pos.copy(),
    ...
    def _do_sample_assignment(self, mship):
        if self.num_samples == 0:
            return
        for c in filter(lambda c: c not in self._cs_sample_assignments.values(), self.cubesats.values()):
    ...
    def _do_cubesat_msg_cb(self):  # noqa D
        for cs, tx_time, msg in self.cubesat_msg_iterator:
            if msgs.Message.is_type(msg, msgs.SampleAquireCommand):
                self._logger.debug("Cubesat %s was commanded to capture sample at %s",
                                   cs.id, msg.msg['sample_pos'])
    ...
    def _do_cubesat_captures(self):  # noqa D
        ...
        for s, c in self._cs_sample_assignments.items():
            if np.linalg.norm(s.position - c.position) <= 1:
                self._logger.debug("Cubesat %s attempting capture at %s",
                                   c.id, s.position)
...
if __name__ == '__main__':
    ...
    def main(): # noqa D
        ...
        if not args.single_threaded:
            pool = ActorPool(get_actor,
                             lambda a: a.run.remote(),
            ...
            DEFAULT_LOGGING = {
                ...
                'handlers': {
                    ...
                    'file':     {'class': 'logging.FileHandler', 
                                 'formatter': "standard", 
                                 'level': 'DEBUG', 
                                 'filename': 'experiment.log','mode': 'w'} 
                },
                'loggers': { 
                    "CaptureClosestSample":   {'level': 'DEBUG', 
            ...
            res = [Experiment(get_sim_config(), args.max_samples, 60*30, logger).run()
